...
-- 初始化数据
INSERT INTO
    roles (code, name, description, is_system)
VALUES
...
WHERE
    r.code = 'AUTHOR'
    AND (
        p.code LIKE 'post:create'
        OR p.code LIKE 'post:update:own'
        OR p.code LIKE 'post:delete:own'
        OR p.code LIKE 'post:read'
    );
